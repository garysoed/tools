...
test('@tools/src/rxjs/state/array-subject', () => {
  test('get', () => {
    ...
    should(
      'emit undefined if there are no elements in the given index',
      async () => {
        const array$ = new ArraySubject([1, 2, 3]);
  ...
  test('push', () => {
    should('push the new element to the array', async () => {
      const array$ = new ArraySubject([1, 2, 3]);
      array$.push(4, 5, 6);
      await asyncAssert(array$).to.emitWith(
        arrayThat<number>().haveExactElements([1, 2, 3, 4, 5, 6]),
      );
    });
  });
  ...
  test('set', () => {
    should('set the element at the given index', async () => {
      ...
      array$.set(1, 6);
      await asyncAssert(array$).to.emitWith(
        arrayThat<number>().haveExactElements([1, 6, 3]),
      );
    });
  });
});
