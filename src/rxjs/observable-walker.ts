...
type SubjectsOf<Object> = {
  readonly [Key in keyof Object]: Object[Key] extends Subject<infer S> ? S : never;
};
...
export interface ImmutableWalker<Object> extends Observable<Object> {
  ...
  $<Key extends keyof SubjectsOf<Object>>(key: Key): MutableWalker<SubjectsOf<Object>[Key]>;
}

class ImmutableWalkerInternal<T> extends Observable<T> implements ImmutableWalker<T> {
  constructor(
  ...
  _<K extends keyof T>(key: K): ImmutableWalker<T[K]> {
    return new ImmutableWalkerInternal(this.source$.pipe(map(value => value[key])));
  }
...
class MutableWalkerInternal<T> extends ImmutableWalkerInternal<T> implements MutableWalker<T> {
  constructor(
      private readonly mutableSource$: Observable<Subject<T>>,
  ) {
