...
test('path.RelativePathParser', () => {
  test('convertBackward', () => {
    should('return the correct path', () => {
      ...
      assert(
        strict(relativePathParser()).convertBackward('a/b/c')!.toString(),
      ).to.equal('a/b/c');
    });

    should('return null if the path is absolute', () => {
      assert(relativePathParser().convertBackward('/a/b/c')).to.haveProperties({
        success: false,
      });
    });

    should('return null if the string is empty', () => {
      assert(relativePathParser().convertBackward('')).to.haveProperties({
        success: false,
      });
    });
  });
  ...
  test('convertForward', () => {
    should('return the correct string', () => {
      const converted = strict(relativePathParser()).convertForward(
