...
test('@tools/collect/compare/compound', () => {
  should(
    'use the first ordering and use the subsequent ones for tie breaking',
    () => {
      const ordering = compound([() => 0 as CompareResult, normal<number>()]);
  ...
  should('return 0 if none of the given orderings can break ties', () => {
    const ordering = compound([() => 0 as CompareResult]);
