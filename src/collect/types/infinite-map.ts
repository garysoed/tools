...
function createInfiniteMap_<K, V>(generator: () => IterableIterator<[K, V]>): InfiniteMap<K, V> {
...
export function createInfiniteMap<V>(
  ...
  if (source instanceof Map || source instanceof Array) {
    return createInfiniteMap_(generatorFrom(source));
  } else {
    ...
    return createInfiniteMap_(generatorFrom(new Map(entries)));
  }
}
