...
import { ANNOTATIONS, Hook } from '../webc/hook';
...
describe('webc.hook', () => {
  ...
  describe('createDecorator_', () => {
    it('should return the correct decorator', () => {
      ...
      const decorator = hook['createDecorator_'](mockBinderFactory);
  ...
  describe('attribute', () => {
    it('should attach the correct factory to the property', () => {
      ...
      hook.attribute(attributeName, parser)(target, propertyKey);
      assert(ANNOTATIONS.forCtor).to.haveBeenCalledWith(ctor);
      ...
      assert(binderFactory.call(hook, element)).to.equal(binder);
  ...
  describe('childrenElement', () => {
    it('should attach the correct factory to the property', () => {
      ...
      hook.childrenElements(dataHelper, startPadCount, endPadCount)(target, propertyKey);
      assert(ANNOTATIONS.forCtor).to.haveBeenCalledWith(ctor);
      assert(mockAnnotationsHandler.attachValueToProperty).to.haveBeenCalledWith(
          propertyKey,
          jasmine.any(Function));
  ...
  describe('innerText', () => {
    it('should return the correct factory to the property', () => {
      const decorator = Mocks.object('decorator');
      spyOn(hook, 'property').and.returnValue(decorator);
      assert(hook.innerText()).to.equal(decorator);
      assert(hook.property).to.haveBeenCalledWith('innerText');
    });
  });
  ...
  describe('property', () => {
    it('should attach the correct factory to the property', () => {
      ...
      hook.property(propertyName)(target, propertyKey);
      assert(ANNOTATIONS.forCtor).to.haveBeenCalledWith(ctor);
      assert(mockAnnotationsHandler.attachValueToProperty).to.haveBeenCalledWith(
          propertyKey,
          jasmine.any(Function));
