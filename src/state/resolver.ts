...
export interface ImmutableResolver<T> extends Observable<T|undefined> {
...
export class ImmutableResolverInternal<T> extends Observable<T|undefined> implements ImmutableResolver<T> {
  constructor(
...
export class MutableResolverInternal<T> extends ImmutableResolverInternal<T> implements MutableResolver<T> {
  constructor(
    ...
    super(mutableSource$.pipe(switchMap(mutable => mutable?.value$ ?? of(undefined))));
  }
